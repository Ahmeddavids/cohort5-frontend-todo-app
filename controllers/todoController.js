...
exports.getAllContent = async (req, res) => {
    try {
        const { userId } = req.user;
        const contents = await TodoModel.find({ user: userId, isDeleted: false});
        res.status(200).json({
...
exports.getAllContentInTrash = async (req, res) => {
    try {
        const { userId } = req.user;
        const contents = await TodoModel.find({ user: userId, isDeleted: true});
        res.status(200).json({
            message: 'All deleted contents found',
            data: contents
        })
    } catch (error) {
        res.status(500).json({
